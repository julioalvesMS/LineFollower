...
void adc_initADCModule(void)
{
   ...
   /* set pin as ADC In */
   PORTB_PCR3 |= PORT_PCR_MUX(TRACK_SENSOR_1_ALT);
   PORTB_PCR2 |= PORT_PCR_MUX(TRACK_SENSOR_2_ALT);
   PORTC_PCR2 |= PORT_PCR_MUX(TRACK_SENSOR_3_ALT);
   PORTB_PCR1 |= PORT_PCR_MUX(TRACK_SENSOR_4_ALT);
   PORTB_PCR0 |= PORT_PCR_MUX(TRACK_SENSOR_5_ALT);
...
/* Output params:      n/a                            */
/* ************************************************** */
void adc_initConvertion(char channel)
{
	ADC0_SC1A |= ADC_SC1_ADCH(0x0f);
    ADC0_SC1A &= (ADC_SC1_ADCH(channel) | ADC_SC1_DIFF(ADC_SC1A_DIFFERENTIAL) | ADC_SC1_AIEN(ADC_SC1A_INTERRUPT));
...
{
    int siConvertionValue = 0;

    adc_initConvertion(sensorId);
...
int adc_read(input_port_type_e enumSensor)
{
	...
	switch(enumSensor){
		case TRACK_SENSOR_5:
			value = adc_converter(8U);
			break;
		case TRACK_SENSOR_4:
			value = adc_converter(9U);
			break;
		case TRACK_SENSOR_3:
			value = adc_converter(11U);
			break;
		case TRACK_SENSOR_2:
			value = adc_converter(12U);
			break;
		case TRACK_SENSOR_1:
			value = adc_converter(13U);
			break;
	}
	return value;
}
